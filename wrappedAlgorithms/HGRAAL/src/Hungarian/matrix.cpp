...
/*export*/template<class T> void Matrix<T>::identity() {
...
/*export*/template<class T> T Matrix<T>::trace() {
	assert( m_matrix != NULL );
...
/*export*/template<class T> Matrix<T>& Matrix<T>::transpose() {
	assert( m_rows > 0 );
	assert( m_columns > 0 );
...
/*export*/template<class T> Matrix<T> Matrix<T>::product(
		Matrix<T> &other) {
	assert( m_matrix != NULL );
