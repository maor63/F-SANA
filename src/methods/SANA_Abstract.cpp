...
void SANA_Abstract::initTau(void) {
    /*    tau = vector<double> {
...
SANA_Abstract::SANA_Abstract(Graph* G1, Graph* G2,
...
Method(G1, G2, name+ "_" +MC->toString())
{
...
SANA_Abstract::~SANA_Abstract() {
    
}


Alignment SANA_Abstract::getStartingAlignment(){
    if(G1->hasNodeTypes()){
...
ushort SANA_Abstract::G1RandomUnlockedNode_Fast(){
    ushort index = G1RandomUnlockedNodeDist(gen);
    return unLockedNodesG1[index];
}

ushort SANA_Abstract::G1RandomUnlockedNode(){
...
ushort SANA_Abstract::G1RandomUnlockedNode(uint source1){ 
    if(!nodesHaveType){
...
ushort SANA_Abstract::G2RandomUnlockedNode(uint target1){
    if(!nodesHaveType){
...
ushort SANA_Abstract::G2RandomUnlockedNode_Fast(){
    ushort node =  G2RandomUnassignedNode(gen);
    return node;
}


void SANA_Abstract::describeParameters(ostream& sout) {
...
string SANA_Abstract::fileNameSuffix(const Alignment& A) {
    return "_" + extractDecimals(eval(A),3);
}

void SANA_Abstract::enableRestartScheme(double minutesNewAlignments, uint iterationsPerStep, uint numCandidates, double minutesPerCandidate, double minutesFinalist) {
    restart = true;
...
double SANA_Abstract::temperatureFunction(double iter, double TInitial, double TDecay) {
...
double SANA_Abstract::acceptingProbability(double energyInc, double T) {
    return energyInc >= 0 ? 1 : exp(energyInc/T);
    //return energyInc >= 0 ? 1 : pBad;
}

double SANA_Abstract::trueAcceptingProbability(){
    return vectorMean(sampledProbability);
}

void SANA_Abstract::initDataStructures(const Alignment& startA) {
    A = startA.getMapping();
...
double SANA_Abstract::eval(const Alignment& Al) {
    return MC->eval(Al);
}

void SANA_Abstract::setInterruptSignal() {
    interrupt = false;
...
void SANA_Abstract::evalAlignmentMethod(){
     ofstream eval_results;
    if(!isFileExists("eval_results.csv")){
        eval_results.open ("eval_results.csv", ofstream::app);
        eval_results << "time_stamp, G1_name,G1_nodes,G1_edges, G2_name,G2_nodes";
        eval_results << ",G2_edges,run_time,method, NC ,EC ,ICS ,S3 , LCCS, OP" <<endl;
    }
    else{
        eval_results.open ("eval_results.csv", ofstream::app);
    }
    ...
    eval_results <<G1->getName() << ",";
    eval_results <<G1->getNumNodes()<< ",";
    eval_results <<G1->getNumEdges()<< ",";
    eval_results <<G2->getName()<< ",";
    eval_results <<G2->getNumNodes()<< ",";
    eval_results <<G2->getNumEdges()<< ",";
    
    eval_results << minutes<< ",";
    eval_results << this->getName()<< ",";
    
    try{
            Alignment a(Alignment::correctMapping(*G1,*G2));
            vector<ushort> mapping = a.getMapping();
            mapping.push_back(G1->getNumNodes());
            eval_results << NodeCorrectness(mapping).eval(A)<< ",";
        }
        catch(exception& e){
            ...
            correctAlignment.push_back(n);
            double nc = NodeCorrectness(correctAlignment).eval(A);
            eval_results << nc << ",";
        }            
    eval_results << EdgeCorrectness(G1, G2).eval(A)<< ",";
    eval_results << InducedConservedStructure(G1, G2).eval(A)<<",";
    eval_results << SymmetricSubstructureScore(G1, G2).eval(A)<< ",";
    eval_results << LargestCommonConnectedSubgraph(G1, G2).eval(A) <<",";
    
    
    vector<string> fields = addFieldsToResults();
    for(int i = 0 ;i < fields.size(); i++){
        eval_results << fields[i]<< ",";     
    }
    
    eval_results << endl;
}

vector<string> SANA_Abstract::addFieldsToResults()
{ 
...
Alignment SANA_Abstract::simpleRun(const Alignment& startA, double maxExecutionSeconds,
    ...
    string timpstamp = to_string(month) + to_string(day) + to_string(hour) + to_string(min);
    string result_file_name = this->getName()+"_"+ G1->getName() + "_"+ G2->getName() + ".csv";
    string path = result_file_name;
    myfile.open (path);
    myfile << "iterations, NC ,EC ,ICS ,S3 ,LCCS" <<endl;    
    for (; ; iter++) {
        ...
        if (iter%iterationsPerStep == 0) {
            trackProgress(iter);  
            cout << "measures start time: " << timer.elapsed()<<endl;
            myfile << iter << ",";
            try{
                Alignment a(Alignment::correctMapping(*G1,*G2));
                vector<ushort> mapping = a.getMapping();
                mapping.push_back(G1->getNumNodes());
                myfile << NodeCorrectness(mapping).eval(A)<< ",";
            }
            catch(exception& e){
                cout << e.what() <<endl;
                map<ushort,string> G1Index2Name = G1->getIndexToNodeNameMap();
		...
		correctAlignment.push_back(n);
            ...
            myfile << EdgeCorrectness(G1, G2).eval(A)<< ",";
            myfile << InducedConservedStructure(G1, G2).eval(A)<<",";
            myfile << SymmetricSubstructureScore(G1, G2).eval(A)<< ",";
            myfile << LargestCommonConnectedSubgraph(G1, G2).eval(A) <<",";

            myfile << endl;
            myfile.flush();
            cout << "measures end time: " << timer.elapsed()<<endl;
...
Alignment SANA_Abstract::simpleRun(const Alignment& startA, long long unsigned int maxExecutionIterations,
        long long unsigned int& iter) {
...
void SANA_Abstract::SANAIteration() {
    iterationsPerformed++;
...
void SANA_Abstract::performChange() {
...
void SANA_Abstract::performSwap() {
...
double SANA_Abstract::scoreComparison(double newAligEdges, double newInducedEdges, double newLocalScoreSum, double newWecSum, double newNcSum, double& newCurrentScore, double newEwecSum) {
...
int SANA_Abstract::aligEdgesIncChangeOp(ushort source, ushort oldTarget, ushort newTarget) {
    int res = 0;
...
int SANA_Abstract::aligEdgesIncSwapOp(ushort source1, ushort source2, ushort target1, ushort target2) {
    int res = 0;
...
int SANA_Abstract::inducedEdgesIncChangeOp(ushort source, ushort oldTarget, ushort newTarget) {
    int res = 0;
...
double SANA_Abstract::localScoreSumIncChangeOp(vector<vector<float> > const & sim, ushort const & source, ushort const & oldTarget, ushort const & newTarget) {
    return sim[source][newTarget] - sim[source][oldTarget];
}

double SANA_Abstract::localScoreSumIncSwapOp(vector<vector<float> > const & sim, ushort const & source1, ushort const & source2, ushort const & target1, ushort const & target2) {
...
double SANA_Abstract::WECIncChangeOp(ushort source, ushort oldTarget, ushort newTarget) {
    double res = 0;
...
double SANA_Abstract::WECIncSwapOp(ushort source1, ushort source2, ushort target1, ushort target2) {
    double res = 0;
...
double SANA_Abstract::EWECIncChangeOp(ushort source, ushort oldTarget, ushort newTarget, const Alignment& A){
...
double SANA_Abstract::EWECIncSwapOp(ushort source1, ushort source2, ushort target1, ushort target2, const Alignment& A){
...
int SANA_Abstract::ncIncChangeOp(ushort source, ushort oldTarget, ushort newTarget) {
    int change = 0;
...
int SANA_Abstract::ncIncSwapOp(ushort source1, ushort source2, ushort target1, ushort target2) {
    int change = 0;
...
void SANA_Abstract::trackProgress(long long unsigned int i) {
...
Alignment SANA_Abstract::runRestartPhases() {
...
uint SANA_Abstract::getLowestIndex() const {
...
uint SANA_Abstract::getHighestIndex() const {
...
void SANA_Abstract::setTemperatureScheduleAutomatically() {
    setTInitialByLinearRegression();
    setTDecayAutomatically();
}

void SANA_Abstract::setTInitialByLinearRegression() {
...
void SANA_Abstract::setTInitialByStatisticalTest() {
    TInitial = searchTInitialByStatisticalTest(); // Nil's code using fancy statistics
    //TInitial = simpleSearchTInitial(); // Wayne's simplistic "make it bigger!!" code
}

void SANA_Abstract::setTDecayAutomatically() {
    if(maxIterations == 0){
...
double SANA_Abstract::searchTInitialByStatisticalTest() {
...
double SANA_Abstract::scoreForTInitial(double TInitial) {
...
double SANA_Abstract::findTInitialByLinearRegression(){
...
string SANA_Abstract::getFolder(){
...
string SANA_Abstract::mkdir(const std::string& file){
...
double SANA_Abstract::pForTInitial(double TInitial) {
...
double SANA_Abstract::getPforTInitial(const Alignment& startA, double maxExecutionSeconds,
        long long unsigned int& iter) {
...
bool SANA_Abstract::isRandomTInitial(double TInitial, double highThresholdScore, double lowThresholdScore) {
...
Alignment SANA_Abstract::hillClimbingAlignment(Alignment startAlignment, long long unsigned int idleCountTarget){
    long long unsigned int iter = 0;
    uint idleCount = 0;
    T = 0;
...
Alignment SANA_Abstract::hillClimbingAlignment(long long unsigned int idleCountTarget){
    long long unsigned int iter = 0;
...
long long unsigned int SANA_Abstract::hillClimbingIterations(long long unsigned int idleCountTarget) {
...
double SANA_Abstract::expectedNumAccEInc(double temp, const vector<double>& energyIncSample) {
    double res = 0;
...
//after hill climbing
vector<double> SANA_Abstract::energyIncSample(double temp) {
...
double SANA_Abstract::simpleSearchTInitial() {
    T = .5e-6;
    double pBad;
    do {
        T *= 2;
...
double SANA_Abstract::searchTDecay(double TInitial, double minutes) {
...
double SANA_Abstract::searchTDecay(double TInitial, uint iterations) {
...
double SANA_Abstract::getIterPerSecond() {
...
void SANA_Abstract::initIterPerSecond() {
...
void SANA_Abstract::setDynamicTDecay() {
    dynamic_tdecay = true; 	
}
