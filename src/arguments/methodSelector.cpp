...
#include "../methods/SANA_Improved.hpp"
#include "../methods/SANA_Abstract.hpp"
...
Method* initSANA(Graph& G1, Graph& G2, ArgumentParser& args, MeasureCombination& M) {
    ...
    if (args.bools["-restart"]) {
        ...
        ((SANA_Abstract*) sana)->enableRestartScheme(tnew, iterperstep, numcand, tcand, tfin);
    }
    if (args.strings["-tinitial"] == "by-linear-regression") {
        ((SANA_Abstract*) sana)->setTInitialByLinearRegression();
    }
    if (args.strings["-tinitial"] == "by-statistical-test") {
        ((SANA_Abstract*) sana)->setTInitialByStatisticalTest();
    }
    if (args.strings["-tdecay"] == "auto") {
        ((SANA_Abstract*) sana)->setTDecayAutomatically();
    }
    if (args.bools["-dynamictdecay"]) {
	((SANA_Abstract*) sana)->setDynamicTDecay();
    } 
...
Method* initSANA_improved(Graph& G1, Graph& G2, ArgumentParser& args, MeasureCombination& M)
{
    ...
    double time = args.doubles["-t"];
    sana = new SANA_Improved(&G1, &G2, TInitial, TDecay, time, args.bools["-usingIterations"], args.bools["-add-hill-climbing"], &M, args.strings["-combinedScoreAs"]);
...
Method* initMethod(Graph& G1, Graph& G2, ArgumentParser& args, MeasureCombination& M) {
    ...
    if (name == "sana_improved")
        return initSANA_improved(G1, G2, args, M);
    if (name == "hc")
